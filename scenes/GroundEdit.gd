...
export(Texture) var texture = preload("res://gfx/Road.png") setget setBakeInterval
export(Texture) var textureL = preload("res://gfx/Road.png")
export(Texture) var textureR = preload("res://gfx/Road.png")
...
func _draw():
	...
	for p in range(get_curve().get_baked_points().size()):
		...
		if p == 0:
			var a = (get_curve().get_point_position(0) - get_curve().get_baked_points()[p]).normalized()
	...
	var startNormal = (get_curve().get_baked_points()[0] - get_curve().get_point_position(0)).rotated(PI/2)/2
	
	var sn1 = get_curve().get_point_position(0) + startNormal
	var sn2 = get_curve().get_baked_points()[0] + normals[0]
	var sn3 = get_curve().get_baked_points()[0] - normals[0]
	var sn4 = get_curve().get_point_position(0) - startNormal
	...
	if(get_parent().has_node("CollisionPolygon2D")):
		#Create collisions
		var collisionPoly = []
		
		#Curve start
		collisionPoly.push_back(get_curve().get_point_position(0))
	...
	if debug:
		...
		if get_curve().get_baked_points().size() > 0:
			draw_line(get_curve().get_point_position(0), get_curve().get_baked_points()[0], Color(0, 0.5, 1), 1)
		...
		#Starting point
		draw_line(get_curve().get_point_position(0), get_curve().get_point_position(0)+startNormal, Color(1, 0.5, 0), 1)
		draw_line(get_curve().get_point_position(0), get_curve().get_point_position(0)-startNormal, Color(1, 0.5, 0), 1)
		draw_circle(get_curve().get_point_position(0), 10, Color(1, 0.5, 0))
